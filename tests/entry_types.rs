...
mod tests {
    ...
    fn it_parses_application() {
        ...
        let file = parse_file(&path_buf).unwrap();
    ...
    fn it_parses_directory() {
        ...
        assert_eq!(file.entry_type, EntryType::Directory);
    ...
    fn it_fails_if_invalid_entry_type() {
        let path_buf: PathBuf = [TEST_FILES_DIR, "test_invalid_entry_type.desktop"].iter().collect();
        let file = parse_file(&path_buf);
